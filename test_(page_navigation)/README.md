...
Этот код открывает первую вкладку методом .get("URL"), затем открывает ещё три вкладки методом .execute_script() и после этого печатает все дескрипторы открытых вкладок.
