...
Selenium реализует drag and drop через класс ActionChains, предоставляя методы:
```drag_and_drop(drag_element, drop_element)``` - позволяет перетаскивать элемент из одного места в другое, указав исходный элемент и целевой элемент.
```click_and_hold(drag_element)``` - предоставляет возможность зажать кнопку мыши на элементе, что важно при осуществлении перетаскивания.
```drag_and_drop_by_offset(x_offset, y_offset)``` -  используется, когда необходимо перетащить элемент на определённое количество пикселей в горизонтальном и вертикальном направлении от его текущей позиции.
```move_by_offset(x_offset, y_offset)```  - используется для перемещения курсора мыши на заданное расстояние относительно текущей позиции.

## Тестовые данные test_A
- [x] Незарегестированный пользователь переходит по ссылке.
- [x] Находит красный бокс и серый бокс.
- [x] Пермещает красный бокс на серый.
#### В программе использует операцию перемещения элемента из одной позиции в другую: 
...
#### В программе использует операцию захвата и удержания элемента: 
...
#### В программе использует перемещение слайдера: 
Цикл по контейнерам:\
...
## Тестовые данные test_K
- [x] Незарегестированный пользователь переходит по ссылке.
- [x] Находит ищет контейнер с кнопками.
- [x] Удерживает каждую найденную кнопку на время полученное из ее атрибута.

#### В программе использует перемещение слайдера: 
```    for element in container_parrent:``` # Проходит в цикле по каждой кнопке в контейнере.
```        timer = element.get_attribute('value')``` # Поучает время удержания кнопки из атрибута 'value'.      
```        ActionChains(driver).click_and_hold(element).pause(float(timer)).release(element).perform()``` # Удерживает кнопку на заданное время, используя ActionChains, затем отпускает элемент и выполняет все действия.